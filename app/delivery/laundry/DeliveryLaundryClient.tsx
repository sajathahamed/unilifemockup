...
export default function DeliveryLaundryClient({ user }: { user: UserProfile }) {
    ...
    return (
        <div className="space-y-6 pb-16">
            <div className="border-b border-gray-200 pb-6 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-semibold text-gray-900">Laundry jobs</h1>
                    <p className="mt-1 text-sm text-gray-600">
                        Collect laundry, record weight, and confirm pickup details.
                    </p>
                </div>
                <div className="flex items-center gap-2 text-sm text-gray-600 bg-card px-3 py-2 rounded-lg border border-gray-200">
                    <span className="w-2 h-2 bg-secondary rounded-full shrink-0" aria-hidden />
                    Coordinator view
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-4">
                    ...
                    {assigned.length > 0 ? (
                        ...
                        <div className="bg-card rounded-card p-10 text-center border border-dashed border-gray-200">
                    ...
                    {confirmed.length > 0 && (
                        <div className="pt-6 space-y-3">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-sm font-semibold text-gray-500">Recently confirmed</h2>
                            </div>
                            {confirmed.map(job => (
                                <div
                                    key={job.id}
                                    className="bg-card p-4 rounded-card border border-gray-200 flex items-center justify-between gap-4"
                                >
                                    ...
                                    <span className="text-xs font-medium text-emerald-800 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-100 shrink-0">
                                        Processing
                                    </span>
                                </div>
                            ))}
                ...
                <div className="lg:col-span-1">
                    <div className="lg:sticky lg:top-24">
                        {selectedJob ? (
                            <div className="bg-card rounded-card p-6 border border-gray-200 shadow-card space-y-5">
                                ...
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                placeholder="0.0"
                                                className="w-full border border-gray-200 rounded-lg py-2.5 pl-10 pr-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
                                            />
                                        </div>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            className="w-full border border-gray-200 rounded-lg p-3 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary h-24 resize-none"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="pt-4 border-t border-gray-100">
                                    ...
                                    <button
                                        ...
                                        className="w-full bg-primary text-white rounded-lg py-2.5 font-medium text-sm hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                    >
                            ...
                            <div className="bg-gray-50 rounded-card p-8 text-center border border-dashed border-gray-200">
            ...
            {showSuccess && confirmedDetails && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
                    <div className="bg-card rounded-card p-6 max-w-md w-full shadow-card border border-gray-200">
                        <div className="text-center space-y-4">
                            ...
                            <button
                                ...
                                className="w-full bg-gray-900 text-white rounded-lg py-2.5 font-medium text-sm hover:bg-gray-800"
                            >
                                Done
                            </button>
                        </div>
...
function JobCard({ job, onAction }: { job: LaundryJob; onAction: () => void }) {
    return (
        <div className="bg-card rounded-card border border-gray-200 shadow-card p-5 hover:shadow-card-hover transition-shadow">
            <div className="flex flex-col md:flex-row gap-5">
                <div className="flex-1 space-y-4">
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="text-xs font-medium text-primary bg-primary/10 px-2 py-1 rounded-md border border-primary/15 shrink-0">
                            Assigned
                        </span>
                    </div>
                ...
                <div className="flex md:flex-col justify-stretch gap-2 md:border-l md:border-gray-100 md:pl-5 pt-2 md:pt-0">
                    <button
                        type="button"
                        onClick={onAction}
                        className="flex-1 md:flex-none px-4 py-2.5 bg-primary text-white rounded-lg font-medium text-sm hover:bg-primary/90 flex items-center justify-center gap-2"
                    >
                        Update pickup
                    </button>
                    <a
                        href={`tel:${job.contact.replace(/\s/g, '')}`}
                        className="px-4 py-2.5 border border-gray-200 text-gray-700 rounded-lg font-medium text-sm hover:bg-gray-50 flex items-center justify-center"
                    >
                        <Phone size={18} />
                    </a>
                </div>
